...
{
    // Convert bluebox:// to bluebox:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bluebox://"))
    {
        uri.replace(0, 12, "bluebox:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BLUEBOX.lnk";
}
...
{
    return GetAutostartDir() / "bluebox.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BLUEBOX-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bluebox-qt [" + tr("command-line options") + "]                     " + "\n";
